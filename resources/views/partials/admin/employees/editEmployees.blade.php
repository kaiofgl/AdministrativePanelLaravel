<div class="container__companies-employes">
    <div class="container__companies-employes__form">
        ...
        <form method="POST" action="{{ route('admin.employees.edit.do') }}" id="form__companies-employes" enctype="multipart/form-data"> 
            @csrf
            ...
            <input type="name" name="name" id="name" value="{{ old('name', $data[0]->name) }}">

            <label for="email"email>E-mail</label>
            <input type="email" name="email" id="email" value="{{ old('email',$data[0]->email) }}">

            <label for="phone">Telefone</label>
            <input type="phone" name="phone" id="phone" value="{{ old('phone',$data[0]->phone) }} " data-mask="(00) 00000-0000">
            
            <label for="cpf">CPF</label>
            <input type="text" name="cpf" id="cpf" onblur="window.teste" value="{{ old('cpf',$data[0]->cpf) }}" data-mask="000.000.000-00">
            ...
            <select name="companies"" id="companies">
                @foreach($data[1]->all() as $companiesDatas)
                    <option value="{{ $companiesDatas->id }}" {{ ($companiesDatas->id == $data[0]->company_id) ? 'selected' : '' }}>{{ $companiesDatas->name }}</option>
                @endforeach
            </select>
